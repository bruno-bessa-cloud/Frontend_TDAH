...
export function TaskCard({ task, onEdit, onDelete, onStatusChange }: TaskCardProps) {
  ...
  return (
    <Card
      className="group hover:shadow-lg transition-all duration-300 border-l-4 hover:scale-[1.02]"
    ...
    >
      ...
      <CardFooter className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t">
        <div className="flex items-center gap-4 text-sm text-gray-600">
          ...
          <div
            className={`flex items-center gap-1.5 ${isOverdue ? 'text-red-600 font-semibold' : ''}`}
          >
        ...
        <div className="flex items-center gap-2">
          {onEdit && (
            <Button variant="outline" size="sm" onClick={() => onEdit(task)} className="h-8">
              Editar
            </Button>
          )}
